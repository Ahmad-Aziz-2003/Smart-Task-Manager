...
export default function RemindersModal({ isOpen, onClose, tasks = [] }) {
  ...
  const todayTasks = tasks.filter((task) => {
    ...
    const hasReminderToday =
      task.reminder &&
      task.reminderTime &&
      isToday(new Date(task.reminderTime));
  ...
  const getTaskType = (task) => {
    ...
    if (isDueToday && hasReminderToday) {
      return {
        type: "both",
        label: "Due today + Reminder today",
      ...
      <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl mx-4 max-h-[80vh] overflow-y-auto">
        ...
        {todayTasks.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500">
              No tasks due today or with reminders set for today.
            </p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="ml-4">
                      <span
                        className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${taskInfo.color}`}
                      >
                        {taskInfo.label}
                      </span>
                    </div>
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          <div className="flex justify-between text-sm text-gray-500">
            ...
            <span>
              {
                todayTasks.filter((task) => isToday(new Date(task.deadline)))
                  .length
              }{" "}
              due today •{" "}
              {
